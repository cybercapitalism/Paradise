...
/obj/item/pda/attackby(obj/item/C, mob/user, params)
	...
	if(istype(C, /obj/item/cartridge) && !cartridge)
		...
		if(!idcard.registered_name)
			to_chat(user, "<span class='notice'>[src] rejects the ID.</span>")
			if(!silent)
		...
		if(!owner)
			...
			if(!HAS_TRAIT(user, TRAIT_HANDS_BLOCKED) && ((src in user) || (isturf(loc) && in_range(src, user))))
				id_check(user, 2)
				to_chat(user, "<span class='notice'>You put the ID into [src]'s slot.<br>You can remove it with ALT click.</span>")
				update_icon(UPDATE_OVERLAYS)
				SStgui.update_uis(src)
