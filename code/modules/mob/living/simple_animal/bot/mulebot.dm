...
/mob/living/simple_animal/bot/mulebot/attackby(obj/item/I, mob/user, params)
	if(istype(I,/obj/item/stock_parts/cell) && open && !cell)
		...
		if(prob(1 + I.force * 2))
			unload(0)
			user.visible_message("<span class='danger'>[user] knocks [load] off [src] with [I]!</span>",
									"<span class='danger'>You knock [load] off [src] with [I]!</span>")
		else
			to_chat(user, "<span class='warning'>You hit [src] with [I] but to no effect!</span>")
			..()
	else
		..()
	update_icon()
