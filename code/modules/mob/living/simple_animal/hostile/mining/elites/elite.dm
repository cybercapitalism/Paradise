...
/obj/structure/elite_tumor/proc/spawn_elite(mob/dead/observer/elitemind)
	...
	if(boosted)
		...
		notify_ghosts("\A [mychild] has been awakened in [get_area(src)]!", enter_link="<a href=?src=[UID()];follow=1>(Click to help)</a>", source = mychild, action = NOTIFY_FOLLOW)
...
/obj/structure/elite_tumor/proc/return_elite()
	...
	if(boosted)
		...
		mychild.grab_ghost()
		notify_ghosts("\A [mychild] has been challenged in [get_area(src)]!", enter_link="<a href=?src=[UID()];follow=1>(Click to help)</a>", source = mychild, action = NOTIFY_FOLLOW)
	INVOKE_ASYNC(src, PROC_REF(arena_checks))
