...
/obj/structure/window/attackby(obj/item/I, mob/living/user, params)
	...
	if(istype(I, /obj/item/stack/rods) && user.a_intent == INTENT_HELP)
		...
		if(isliving(G.affecting))
			...
			switch(state)
				if(1)
					M.visible_message("<span class='warning'>[user] slams [M] against [src]!</span>")
					M.apply_damage(7)
					take_damage(10)
				if(2)
					M.visible_message("<span class='danger'>[user] bashes [M] against [src]!</span>")
					if(prob(50))
						M.Weaken(2 SECONDS)
					M.apply_damage(10)
					take_damage(25)
				if(3)
					M.visible_message("<span class='danger'><big>[user] crushes [M] against [src]!</big></span>")
					M.Weaken(10 SECONDS)
					M.apply_damage(20)
					take_damage(50)
				if(4)
					visible_message("<span class='danger'><big>[user] smashes [M] against [src]!</big></span>")
					M.Weaken(10 SECONDS)
					M.apply_damage(30)
					take_damage(75)
	else
